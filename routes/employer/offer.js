  ...
  try {
    ...
    if (!errors) {
      ...
      if (user && user.data.role_id == "5d9d99003a0c78039c6dd00f") {
        ...
        employer = await common_helper.findOne(SubEmployerDetail, {
          emp_id: user.data.emp_id
        });

        var company_resp = await common_helper.findOne(EmployerDetail, {
        ...
        employer = await common_helper.findOne(EmployerDetail, {
      ...
      var candidate_user = await common_helper.find(User, {
        email: req.body.email.toLowerCase(),
        is_del: false
      });
      if (candidate_user.data.length <= 0) {
        var interest_candidate = await common_helper.insert(User, {
          email: req.body.email.toLowerCase(),
          role_id: "5d9d98e13a0c78039c6dd00e"
        });
        ...
        var interest_candidate_detail = await common_helper.insert(
          CandidateDetail,
          newcandidate
        );
        obj.user_id = interest_candidate.data._id;
        var role = await common_helper.findOne(Role, {
          _id: interest_candidate.data.role_id
        });
      } else {
        obj.user_id = candidate_user.data[0]._id;
        var role = await common_helper.findOne(Role, {
          _id: candidate_user.data[0].role_id
        });
      }
      ...
      if (role.data.role !== "candidate") {
        res
          ...
          .json({ message: "You can not send offer to this user." });
      } else {
        var pastOffer = await common_helper.find(Offer, {
          user_id: ObjectId(obj.user_id),
          status: "Not Joined",
          $or: [
            { employer_id: new ObjectId(requested_user) },
            { employer_id: new ObjectId(requested_user) }
          ]
        });
        ...
        if (interest_resp.status == 0) {
          ...
          let employer_role = await common_helper.findOne(User, {
            _id: employer.data.user_id
          });
          var companyname;
          if (employer_role.data.role_id == "5d9d99003a0c78039c6dd00f") {
            let contact_name = await common_helper.findOne(EmployerDetail, {
              user_id: employer.data.emp_id
            });
            companyname = contact_name.data.companyname;
          } else if (employer_role.data.role_id == "5d9d98a93a0c78039c6dd00d") {
            let contact_name = await common_helper.findOne(EmployerDetail, {
              user_id: employer.data.user_id
            });
            companyname = contact_name.data.companyname;
          }

          var user = await common_helper.findOne(User, {
            _id: new ObjectId(interest_resp.data.user_id)
          });
          var candidate = await common_helper.findOne(CandidateDetail, {
            user_id: new ObjectId(interest_resp.data.user_id)
          });
          var status = await common_helper.findOne(Status, {
            status: "On Hold"
          });
          var mailcontent = await common_helper.findOne(MailContent, {
            mail_type: "offer_mail"
          });
          ...
          upper_content = upper_content
            .replace("{employername}", `${companyname}`)
            .replace(
              "{offer_expiry_date}",
              `${moment(interest_resp.data.expirydate)
                .startOf("day")
                .format("DD/MM/YYYY")}`
            );
          ...
          var reply_to = await common_helper.findOne(User, {
            _id: interest_resp.data.created_by
          });
          ...
          if (interest_resp.data.status === "On Hold") {
            ...
            for (let index = 0; index < all_employer.data.length; index++) {
              ...
              if (element.role_id == "5d9d99003a0c78039c6dd00f") {
                var emp_name = await common_helper.findOne(SubEmployerDetail, {
                  user_id: new ObjectId(element._id)
                });
                ...
                var emp_name = await common_helper.findOne(EmployerDetail, {
                  user_id: new ObjectId(element._id)
                });
                var email = emp_name.data.username;
                var name = email.substring(0, email.lastIndexOf(" "));
              ...
              let mail_resp = await mail_helper.send(
                "on_hold_offer",
                {
                  ...
                  subject:
                    ...
                    " offer created in On Hold status"
                },
                {
                  name: name,
                  upper_content: upper_content,
            ...
            let mail_resp = await new_mail_helper.sendOffer(
              "d-96c1114e4fbc45458f2039f9fbe14390",
              {
                to: user.data.email,
            ...
            if (mail_resp.status == 1) {
              ...
              var candidate_email = await common_helper.findOne(User, {
                _id: interest_resp.data.user_id
              });
              var candidate_name = await common_helper.findOne(
                CandidateDetail,
                { user_id: interest_resp.data.user_id }
              );
              var location = await common_helper.findOne(Location, {
                _id: interest_resp.data.location
              });

              if (
                interest_resp.data !== undefined &&
                interest_resp.data.communication.length > 0
              ) {
                for (const comm of interest_resp.data.communication) {
                  if (
                    comm.trigger == "afterOffer" &&
                    comm.day == 0 &&
                    comm.mail_send == false
                  ) {
                    if (
                      moment(interest_resp.data.createdAt)
                        .startOf("day")
                        .isSame(current_date)
                    ) {
                      ...
                      message = message
                        ...
                        .replace(
                          /\|\|candidate_name\|\|/g,
                          `${candidate_name.data.firstname +
                            " " +
                            candidate_name.data.lastname}`
                        )
                        .replace(/\|\|title\|\|/g, interest_resp.data.title)
                        .replace(/\|\|location\|\|/g, location.data.city)
                        .replace(
                          /\|\|joining_date\|\|/g,
                          moment(interest_resp.data.joiningdate)
                            .startOf("day")
                            .format("DD/MM/YYYY")
                        )
                        .replace(
                        ...
                        .replace(
                          /\|\|acceptance_date\|\|/g,
                          moment(interest_resp.data.acceptedAt)
                            .startOf("day")
                            .format("DD/MM/YYYY")
                        );
                      ...
                      let mail_resp = await communication_mail_helper.send(
                        "d-e3cb56d304e1461d957ffd8fe141819c",
                        {
                          to: candidate_email.data.email,
                          reply_to: `${interest_resp.data._id +
                            "_" +
                            comm._id +
                            "_" +
                            "communication"}@em7977.hirecommit.com`,
                          subject: comm.subject,
                          trackid:
                            interest_resp.data._id +
                            "_" +
                            comm._id +
                            "_" +
                            "communication"
                      ...
                      if (mail_resp.status == 1) {
                        var update_offer_communication = await common_helper.update(
                          Offer,
                          {
                            _id: interest_resp.data._id,
                            "communication._id": comm._id
                          },
                          {
                            $set: {
                    ...
                    comm.trigger == "beforeExpiry" &&
                    comm.day >= 0 &&
                    comm.mail_send == false
                  ) {
                    var offer_date = moment(interest_resp.data.expirydate)
                      .startOf("day")
                      .subtract(comm.day, "day");
              ...
              if (
                interest_resp.data.AdHoc !== undefined &&
                interest_resp.data.AdHoc.length > 0
              ) {
                for (const comm of interest_resp.data.AdHoc) {
                  if (
                    comm.AdHoc_trigger == "afterOffer" &&
                    comm.AdHoc_day == 0 &&
                    comm.AdHoc_mail_send == false
                  ) {
                    if (
                      moment(interest_resp.data.createdAt)
                        .startOf("day")
                    ...
                    comm.AdHoc_trigger == "beforeExpiry" &&
                    comm.AdHoc_day >= 0 &&
                    comm.AdHoc_mail_send == false
                  ) {
                    var offer_date = moment(interest_resp.data.expirydate)
                      .startOf("day")
                      .subtract(comm.AdHoc_day, "day");
                    offer_date = moment(offer_date);
                    if (moment(offer_date).startOf("day") <= current_date) {
                      var message = comm.AdHoc_message;
                      message = message
                    ...
                    comm.AdHoc_trigger == "afterExpiry" &&
                    comm.AdHoc_day == 0 &&
                    comm.AdHoc_mail_send == false
                  ) {
                    if (
...
router.post("/pastOffer", async (req, res) => {
  try {
    ...
    if (user.status == 1) {
      ...
      if (pastOffer.data.length > 0) {
        var display_message = await common_helper.findOne(DisplayMeaasge, {
          msg_type: "past_offer"
        });
    ...
    return res
      ...
      .json({
        ...
        ReleasedOffer: ReleasedOffer
      });
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
      .json({ message: "Error occurred while fetching", status: 0 });
  }
});
...
router.post("/check_is_candidate", async (req, res) => {
  try {
    ...
    if (validator.validate(value) == true) {
      ...
      if (
        user.status == 2 ||
        (user.status == 1 && user.data.role_id == "5d9d98e13a0c78039c6dd00e")
      ) {
        res
          .status(config.OK_STATUS)
          .json({ status: 1, message: "valid candidate" });
      } else {
        res
          .status(config.BAD_REQUEST)
          .json({ status: 2, message: "You can not sent offer to this user." });
      }
    } else {
      res
        .status(config.BAD_REQUEST)
        .json({ status: 2, message: "Email is not valid" });
    }
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
...
cron.schedule("00 00 * * *", async (req, res) => {
  try {
        ...
        for (const comm of resp.communication) {
          if (comm.open === false) {
            if (comm.trigger == "afterOffer") {
              var days = comm.day;
              var offer_date = moment(resp.createdAt)
                .startOf("day")
                .add(days, "day");
              offer_date = moment(offer_date);
              if (
                comm.priority === "High" &&
                (resp.high_unopened !== 0 || resp.high_unopened !== null)
              ) {
                var NotOpenDay = moment(offer_date)
                  .startOf("day")
                  .add(resp.high_unopened, "day");
                if (moment(current_date).isSame(NotOpenDay) == true) {
                  var data = {
                    candidatename:
                      resp.candidate.firstname + " " + resp.candidate.lastname,
                ...
                (resp.medium_unopened !== 0 || resp.medium_unopened !== null)
              ) {
                var NotOpenDay = moment(offer_date)
                  .startOf("day")
                  .add(resp.medium_unopened, "day");
                if (moment(current_date).isSame(NotOpenDay) == true) {
                  var data = {
                    candidatename:
                      resp.candidate.firstname + " " + resp.candidate.lastname,
              ...
              var days = comm.day;
              var offer_date = moment(resp.joiningdate)
                .startOf("day")
                .subtract(days, "day");
              offer_date = moment(offer_date);
              if (
              ...
              var days = comm.day;
              var offer_date = moment(resp.expirydate)
                .startOf("day")
                .subtract(days, "day");
              offer_date = moment(offer_date);
              if (
              ...
              var days = comm.day;
              var offer_date = moment(resp.acceptedAt)
                .startOf("day")
                .add(days, "day");
              offer_date = moment(offer_date);
              if (
          ...
          if (comm.reply === false) {
            if (comm.trigger == "afterOffer") {
              ...
              if (
                ...
                var NotReplyDay = moment(offer_date)
                  .startOf("day")
                  .add(resp.high_notreplied, "day");
                ...
                var NotReplyDay = moment(offer_date)
                  .startOf("day")
                  .add(resp.medium_notreplied, "day");
                if (moment(current_date).isSame(NotReplyDay) == true) {
                  var data = {
                    candidatename:
                      resp.candidate.firstname + " " + resp.candidate.lastname,
    ...
    var all_employer = await common_helper.find(User, {
      role_id: "5d9d98a93a0c78039c6dd00d"
    });
    ...
    if (all_employer.status == 1) {
      for (const filterdata of all_employer.data) {
        if (communication_notopen.length > 0 || adhoc_notopen.length > 0) {
          let filtered = communication_notopen.filter(
            r => r.empid.toString() === filterdata._id.toString()
          );
          let filtered1 = adhoc_notopen.filter(
            r => r.empid.toString() === filterdata._id.toString()
          );
          ...
          if (filtered.length > 0 || filtered1.length > 0) {
            ...
            if (email_send_to.status == 1) {
              for (const sendto of email_send_to.data) {
                ...
                if (element.role_id == "5d9d99003a0c78039c6dd00f") {
                  var emp_name = await common_helper.findOne(
                    SubEmployerDetail,
                    { user_id: new ObjectId(element._id) }
                  );
                  var email = emp_name.data.username;
        ...
        if (communication_notreply.length > 0 || adhoc_notreply.length > 0) {
          let filtered = communication_notreply.filter(
            r => r.empid.toString() === filterdata._id.toString()
          );
          let filtered1 = adhoc_notreply.filter(
            r => r.empid.toString() === filterdata._id.toString()
          );
          ...
          if (filtered.length > 0 || filtered1.length > 0) {
            ...
            if (email_send_to.status == 1) {
              for (const sendto of email_send_to.data) {
                ...
                let mail_resp = await mail_helper.send(
                  "not_replied_communication_mail",
                  {
                    to: element.email,
      ...
      return res
        .status(config.BAD_REQUEST)
        .json({ message: "Employer Not Found", success: false });
    }
  } catch (error) {
    ...
    for (const resp of resp_data) {
      if (
        ...
        for (const element of all_employer.data) {
          ...
          var message = await common_helper.findOne(MailContent, {
            mail_type: "candidate_has_joined"
          });
          ...
          upper_content = upper_content
            ...
            .replace("{title}", resp.title)
            .replace("{location}", location.data.city);

          let mail_resp = await mail_helper.send(
            "candidate_has_joined",
            {
              to: element.email,
              subject:
                `${resp.candidate.firstname + " " + resp.candidate.lastname}` +
        ...
        moment(resp.expirydate)
          .startOf("day")
          .subtract(1, "day")
          .isSame(current_date)
      ) {
        var employer_name = await common_helper.findOne(EmployerDetail, {
          user_id: resp.created_by._id
        });
        var employer_name1 = await common_helper.findOne(SubEmployerDetail, {
          user_id: resp.created_by._id
        });
        var companyname;
        if (employer_name.status == 1) {
          ...
          let contact_name = await common_helper.findOne(EmployerDetail, {
            user_id: employer_name1.data.emp_id
          });
          companyname = contact_name.data.companyname;
        }

        var message = await common_helper.findOne(MailContent, {
          mail_type: "offer_expiring"
        });
        ...
        let mail_resp = await mail_helper.send(
          "offer_expiring",
          {
            ...
            subject:
              "Your offer from " + `${companyname}` + " is expiring soon!!"
          },
          {
        ...
        for (const element of all_employer.data) {
          ...
          var message = await common_helper.findOne(MailContent, {
            mail_type: "offer_expiring_employer"
          });
          var upper_content = message.data.upper_content;
          var middel_content = message.data.middel_content;
          var lower_content = message.data.lower_content;
...
cron.schedule("00 00 * * *", async (req, res) => {
  try {
    ...
    for (const resp of resp_data) {
      var companyname_resp = await common_helper.findOne(EmployerDetail, {
        user_id: resp.employer_id
      });
      var companyname;
      ...
      if (resp.AdHoc !== undefined && resp.AdHoc.length > 0) {
        for (const comm of resp.AdHoc) {
          ...
          if (
            comm.AdHoc_trigger == "beforeJoining" &&
            comm.AdHoc_mail_send == false &&
            resp.status == "Accepted"
          ) {
            var days = comm.AdHoc_day;
          ...
          if (
            comm.AdHoc_trigger == "afterJoining" &&
            comm.AdHoc_mail_send == false &&
            resp.status == "Accepted"
          ) {
            var days = comm.AdHoc_day;
          ...
          if (
            comm.AdHoc_trigger == "afterAcceptance" &&
            comm.AdHoc_mail_send == false &&
            resp.status == "Accepted"
          ) {
            var days = comm.AdHoc_day;
...
router.post("/get", async (req, res) => {
  try {
    ...
    if (!errors) {
        ...
        aggregate.push({
          $match: {
            $or: [
              ...
              { commitstatus: RE },
              { customfeild1: RE },
              { "candidate.user.email": RE },
              { "candidate.firstname": RE }
            ]
          }
        });
      }
      ...
      var resp_data = await offer_helper.get_all_offer(
        Offer,
        user_id,
        req.body.search,
        req.body.start,
...
router.put("/status_change", async (req, res) => {
  try {
    ...
    var update_status = await common_helper.update(
      Offer,
      { _id: req.body.id },
      obj
    );
    ...
    content =
      update_status.data.title + "'s status has been changed to " + obj.status;
    ...
    res
      .status(config.OK_STATUS)
      .json({ message: "Status is changed successfully", status: obj.status });
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
      .json({ message: error.message, success: false });
...
router.put("/deactive_offer/:id", async (req, res) => {
  try {
    ...
    if (resp_data.status == 0) {
      ...
      res
        .status(config.OK_STATUS)
        .json({ message: "Offer is Updated successfully", resp_data });
    } else {
      res
        .status(config.BAD_REQUEST)
        .json({ status: 2, message: "Error occurred while fetching data." });
    }
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
...
router.put("/", async (req, res) => {
  try {
    ...
    if (req.body.salary == "") {
      var unset_salary = await Offer.update(
        { _id: ObjectId(id) },
        { $unset: { salary: "" } }
      );
      ...
      var unset_salary = await Offer.update(
        { _id: ObjectId(id) },
        { $unset: { salary_from: "", salary_to: "" } }
      );
    }

    if (req.body.groups === "") {
      var unset_groups = await Offer.updateOne(
        { _id: ObjectId(id) },
        { $unset: { groups: "" } }
      );
    }
    var offer_upadate = await common_helper.update(
      Offer,
      { _id: ObjectId(id) },
      obj
    );
    var user_email = await common_helper.findOne(User, {
      _id: offer_upadate.data.user_id
    });
    ...
    if (offer_upadate.status == 0) {
      ...
      if (
        offer.data.status === "Released" &&
        offer_upadate.data.status === "Released"
      ) {
        var message = await common_helper.findOne(MailContent, {
          mail_type: "update_offer"
        });
        ...
        upper_content = upper_content
          ...
          .replace(
            "{expirydate}",
            moment(offer_upadate.data.expirydate)
              .startOf("day")
              .format("DD/MM/YYYY")
          );
        var user_name = await common_helper.findOne(CandidateDetail, {
          user_id: offer_upadate.data.user_id
        });
        ...
        offer_upadate.data.status === "Not Joined" &&
        offer_upadate.data.offertype !== "noCommit"
      ) {
        var message = await common_helper.findOne(MailContent, {
          mail_type: "not_join_offer"
        });
        ...
        let mail_resp = await mail_helper.send(
          "not_joined_offer",
          {
            ...
            subject:
              companyname +
              " has marked that you have Not Joined after accepting an offer"
          },
          {
            name: name,
            upper_content: upper_content,
        ...
        var reply_to = await common_helper.findOne(User, {
          _id: offer_upadate.data.created_by
        });
        let mail_resp = await new_mail_helper.sendOffer(
          "d-96c1114e4fbc45458f2039f9fbe14390",
          {
        ...
        if (mail_resp.status == 1) {
          var update_date = await common_helper.update(
            Offer,
            { _id: offer_upadate.data._id },
            { createdAt: new Date() }
          );

          var current_date = moment().startOf("day");
          var candidate_email = await common_helper.findOne(User, {
            _id: update_date.data.user_id
          });
          var candidate_name = await common_helper.findOne(CandidateDetail, {
            user_id: update_date.data.user_id
          });
          var location = await common_helper.findOne(Location, {
            _id: update_date.data.location
          });

          if (
            update_date.data !== undefined &&
            update_date.data.communication.length > 0
          ) {
            for (const comm of update_date.data.communication) {
              if (
                ...
                if (
                  moment(update_date.data.createdAt)
                    .startOf("day")
                    .isSame(current_date)
                ) {
                  var message = comm.message;
                  message = message
                    ...
                    .replace(/\|\|title\|\|/g, update_date.data.title)
                    .replace(/\|\|location\|\|/g, location.data.city)
                    .replace(
                      /\|\|joining_date\|\|/g,
                      moment(update_date.data.joiningdate)
                        .startOf("day")
                        .format("DD/MM/YYYY")
                    )
                    .replace(
                      /\|\|expiry_date\|\|/g,
                      moment(update_date.data.expirydate)
                        .startOf("day")
                        .format("DD/MM/YYYY")
                    )
                    .replace(
                      /\|\|acceptance_date\|\|/g,
                      moment(update_date.data.acceptedAt)
                        .startOf("day")
                        .format("DD/MM/YYYY")
                    );
                ...
                var offer_date = moment(update_date.data.expirydate)
                  .startOf("day")
                  .subtract(comm.day, "day");
                offer_date = moment(offer_date);
                if (moment(offer_date).startOf("day") <= current_date) {
          ...
          if (
            update_date.data.AdHoc !== undefined &&
            update_date.data.AdHoc.length > 0
          ) {
                ...
                if (
                  ...
                  let mail_resp = await communication_mail_helper.send(
                    "d-e3cb56d304e1461d957ffd8fe141819c",
                    {
                      ...
                      subject: comm.AdHoc_subject,
                      trackid:
                        update_date.data._id + "_" + comm._id + "_" + "adhoc"
                    },
                    obj
                  );
                  if (mail_resp.status == 1) {
...
router.get("/details/:id", async (req, res) => {
  ...
  try {
    ...
    var candidate_detail = await common_helper.findOne(CandidateDetail, {
      user_id: offer_detail.user_id._id
    });
    return res
      .status(config.OK_STATUS)
      .json({
        ...
        status: 1,
        data: offer_detail,
        candidate_data: candidate_detail
      });
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
  ...
  try {
    ...
    for (let index = 0; index < history_data.length; index++) {
      ...
      if (element.employer_id && element.employer_id != undefined) {
        var employer = await common_helper.findOne(EmployerDetail, {
          user_id: element.employer_id
        });

        var sub_employer = await common_helper.findOne(SubEmployerDetail, {
          user_id: element.employer_id
        });

        var candidate = await common_helper.findOne(CandidateDetail, {
          user_id: element.offer.user_id
        });

        var user = await common_helper.findOne(User, {
          _id: element.offer.user_id
        });
        ...
        if (
          ...
          if (
            ...
            content = content
              .replace("{employer}", `${employer.data.username} `)
              .replace(
                "{candidate}",
                candidate.data.firstname + " " + candidate.data.lastname
              );
            ...
            content = content
              .replace("{employer}", `${employer.data.username} `)
              .replace("{candidate}", candidate.data.firstname);
            ...
            content = content
              .replace("{employer}", `${employer.data.username} `)
              .replace("{candidate}", user.data.email);
          ...
          if (
            ...
            content = content
              .replace("{employer}", `${sub_employer.data.username} `)
              .replace(
                "{candidate}",
                candidate.data.firstname + " " + candidate.data.lastname
              );
    ...
    if (history_data) {
      return res
        .status(config.OK_STATUS)
        .json({ message: "Offer history", status: 1, data: history });
    }
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
...
router.get("/status_list/:status", async (req, res) => {
  try {
    ...
    if (status === "On Hold") {
      ...
      obj.status = [{ label: "Inactive", value: "Inactive" }];
    }
    res
      .status(config.OK_STATUS)
      .json({ message: "Status is changed successfully", status: obj.status });
