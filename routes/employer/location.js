...
router.post("/", async (req, res) => {
  try {
    ...
    if (!errors) {
      ...
      var location_resp = await common_helper.findOne(location, {
        is_del: false,
        emp_id: reg_obj.emp_id,
        city: regex
      });
...
router.post("/get", async (req, res) => {
  try {
    ...
    if (!errors) {
      ...
      var resp_data = await location_helper.get_all_location(
        location,
        user_id,
        req.body.search,
        req.body.start,
...
router.get("/get_location", async (req, res) => {
  try {
    ...
    return res
      ...
      .json({ message: "Location List", status: 1, data: location_list });
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
...
router.get("/get_locations", async (req, res) => {
  try {
    ...
    var location_list = await location
      .find({
        emp_id: new ObjectId(user_id),
        is_del: false
      })
    ...
    if (location_list && location_list.length > 0) {
      ...
      return res
        .status(config.OK_STATUS)
        .json({ message: "No Record Found", status: 2 });
    } else {
      return res
        .status(config.BAD_REQUEST)
        .json({ message: "Error occurred while fetching", status: 0 });
...
router.put("/", async (req, res) => {
  try {
    ...
    if (exist_location.status == 2) {
      var update_location = await common_helper.update(
        location,
        { _id: id },
        reg_obj
      );
      ...
      res
        .status(config.BAD_REQUEST)
        .json({ status: 0, message: "This location is already exist" });
    }
  } catch (error) {
    return res
      .status(config.BAD_REQUEST)
  ...
  try {
    ...
    if (resp_data && resp_data.length > 0) {
      res.status(config.BAD_REQUEST).json({
        status: 0,
        message: "This Location can't be deleted because it is used in offer."
      });
    } else {
      ...
      if (resp_data.status == 0) {
        ...
        res
          .status(config.OK_STATUS)
          .json({ message: "Location is Deleted successfully", resp_data });
      }
    }
  } catch (error) {
    return res
...
router.get("/:id", async (req, res) => {
  try {
    var id = req.params.id;
    var location_detail = await common_helper.findOne(location, {
      _id: ObjectId(id)
    });
    if (location_detail.status == 0) {
      ...
      res.status(config.OK_STATUS).json({
        status: 1,
        message: "Location Details are fetched successfully",
